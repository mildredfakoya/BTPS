...
$(document).ready(function() {
  $(".submit").validate({
	...
	$.ajax({
		  url:"submitcontinous7.php",
		  method:"post",
		  data:$('form').serialize(),
		  dataType:"text",
      success:function(){
       //alert('Submitted');
       location.assign('grade7.php');
}
...
foreach($stmtmulti as $rowmulti){
  $sqlcheck= "SELECT * FROM btps_student_continous_grade_7 WHERE email = :email AND assessment_id = :id AND question_id = :qid";
