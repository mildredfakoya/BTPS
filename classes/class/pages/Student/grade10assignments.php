...
<div class ="jumbotron">
<div class ="header" style ="background-color:#ff0000"><h3>Welcome to Grade 10 Graded Assessment page</h3>
...
<h5 class ="header">Assignments</h5>
  ...
  <?php
    $targetclass = "grade_10";
    $sqlclass="SELECT * FROM btps_new_assessment WHERE target_class= :class AND (assessment_type = 'assignment' || assessment_type = 'project') AND approval_status ='Approved' AND term =:term AND academic_year = :year AND access_status = 'UNLOCKED'" ;
    ...
    foreach($stmtclass as $rowclass){
      ...
      if($diffdate <= 0){
      echo "<form method ='post' action ='grade10assignmentform.php' id ='getassignment' novalidate = 'novalidate'>";
      echo "<div class ='row'>";
  ...
  $sqlcheck= "SELECT DISTINCT assessment_id FROM btps_student_assignment_grade_10 WHERE email = :email";
