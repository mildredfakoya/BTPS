...
$(document).ready(function() {
  $(".submit").validate({
	...
	$.ajax({
		  url:"submitcontinous9.php",
		  method:"post",
		  data:$('form').serialize(),
		  dataType:"text",
      success:function(){
       //alert('Submitted');
       location.assign('grade9.php');
}
...
if($rowaccess['access_code'] == $accesscode){
  ...
  if(!empty($rowtimer)){
      ...
      echo "<div class ='alert alert-success'>Our Records show that you have previously viewed or submitted this assessment. For further assisstance, contact administration</div>";
...
foreach($stmtmulti as $rowmulti){
  $sqlcheck= "SELECT * FROM btps_student_continous_grade_9 WHERE email = :email AND assessment_id = :id AND question_id = :qid";
