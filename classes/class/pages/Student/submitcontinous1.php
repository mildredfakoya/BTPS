...
for($i=0;$i<count($questionid);$i++)
{
  ...
  $sqlcheck = "SELECT * FROM btps_student_continous_grade_1 WHERE question_id = '$questionid[$i]' AND email = '$email'";
...
try{
  $sqlmultichoice= "UPDATE btps_student_continous_grade_1 SET submitted_at ='$submittedat', student_answer ='$studentanswer[$i]' WHERE question_id = '$questionid[$i]' AND email ='$email'";
  ...
  try{
  $sqlsubmit = "INSERT INTO btps_student_continous_grade_1(submitted_at, email, firstname, lastname, assessment_id, question_id, subject,
