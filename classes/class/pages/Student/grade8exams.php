...
<h5 class ="header">Examination</h5>
  ...
  $rowcurrent = $stmtcurrent->fetch(PDO::FETCH_ASSOC);
    ...
    foreach($stmtclass as $rowclass){
      ...
      if($diffdate <= 0){
      echo "<form method ='post' action ='grade8examform.php' id ='getassignment' novalidate = 'novalidate'>";
      echo "<div class ='row'>";
  ...
  $sqlcheck= "SELECT DISTINCT assessment_id FROM btps_student_exam_grade_8 WHERE email = :email";
