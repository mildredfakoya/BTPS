...
<h5 class ="header">Examination</h5>
  ...
  $rowcurrent = $stmtcurrent->fetch(PDO::FETCH_ASSOC);
    ...
    $targetclass = "grade_6";
    $sqlclass="SELECT * FROM btps_new_assessment WHERE target_class= :class and assessment_type = 'exam' and approval_status ='Approved' AND term =:term AND academic_year = :year AND access_status = 'UNLOCKED'" ;
    ...
    foreach($stmtclass as $rowclass){
      ...
      if($diffdate <= 0){
      echo "<form method ='post' action ='grade6examform.php' id ='getassignment' novalidate = 'novalidate'>";
      echo "<div class ='row'>";
  ...
  $sqlcheck= "SELECT DISTINCT assessment_id FROM btps_student_exam_grade_6 WHERE email = :email";
