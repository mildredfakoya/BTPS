...
<h5 class ="header">Continous Assessment</h5>
  ...
  $rowcurrent = $stmtcurrent->fetch(PDO::FETCH_ASSOC);
    ...
    foreach($stmtclass as $rowclass){
      ...
      if($diffdate <= 0){
      echo "<form method ='post' action ='grade1continousform.php' id ='getassignment' novalidate = 'novalidate'>";
      echo "<div class ='row'>";
  ...
  $sqlcheck= "SELECT DISTINCT assessment_id FROM btps_student_continous_grade_1 WHERE email = :email";
