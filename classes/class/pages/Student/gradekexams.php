...
<h5 class ="header">Examination</h5>
  ...
  <?php
    $targetclass = "grade_k";
    $sqlclass="SELECT * FROM btps_new_assessment WHERE target_class= :class and assessment_type = 'exam' AND term =:term AND academic_year = :year AND approval_status ='Approved'" ;
    ...
    foreach($stmtclass as $rowclass){
      ...
      if($diffdate <= 0){
      echo "<form method ='post' action ='gradekexamform.php' id ='getassignment' novalidate = 'novalidate'>";
      echo "<div class ='row'>";
  ...
  $sqlcheck= "SELECT DISTINCT assessment_id FROM btps_student_exam_grade_k WHERE email = :email";
