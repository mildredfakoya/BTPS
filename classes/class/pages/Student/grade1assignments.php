...
<h5 class ="header">Assignments</h5>
  ...
  $rowcurrent = $stmtcurrent->fetch(PDO::FETCH_ASSOC);
    ...
    foreach($stmtclass as $rowclass){
      ...
      if($diffdate <= 0){
      echo "<form method ='post' action ='grade1assignmentform.php' id ='getassignment' novalidate = 'novalidate'>";
      echo "<div class ='row'>";
  ...
  $sqlcheck= "SELECT DISTINCT assessment_id FROM btps_student_assignment_grade_1 WHERE email = :email";
