...
for($i=0;$i<count($questionid);$i++)
{
  ...
  $sqlcheck = "SELECT * FROM btps_student_continous_pre_k WHERE question_id = '$questionid[$i]' AND email = '$email'";
...
try{
  $sqlmultichoice= "UPDATE btps_student_continous_pre_k SET submitted_at ='$submittedat', student_answer ='$studentanswer[$i]' WHERE question_id = '$questionid[$i]' AND email ='$email'";
  ...
  try{
  $sqlsubmit = "INSERT INTO btps_student_continous_pre_k(submitted_at, email, firstname, lastname, assessment_id, question_id, subject,
