...
$sqlletter = 'SELECT * FROM ihs_newsletter_uploads ORDER BY created_at DESC';
$stmtletter = $user_home->runQuery($sqlletter);
$stmtletter->execute();
...
foreach($stmtletter as $rowletter){
...
<tr>
  ...
  <td><input type='hidden' name='hiddeninfo' value='<?php echo $rowletter['file']?>'><input type='submit' name='deleteinfo' value='Delete' class ='btn btn-danger' style ='width:100%'/></td>
</tr>
  </table>
</form>
<?php
            ...
            if(isset($_POST['deleteinfo'])){
               	try{
                    $sql = "DELETE FROM ihs_newsletter_uploads WHERE file='$_POST[hiddeninfo]'";
                    $result = $user_home->runQuery4($sql);
                   if ($result){
                                   unlink($_POST['hiddeninfo']);
